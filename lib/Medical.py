...
def main():
    ...
    st.title("Clinical Trial Search Engine")  # Title of the app
    
    st.markdown("---")  # A horizontal line for separation

    st.markdown("""
        ## Brief Instruction   
        This tool allows you to find relevant clinical trials based on your search query. 
        It uses machine learning models—**Skipgram** and **FastText**—to analyse the textual context and find the top and most relevant results for your input. 
        
        ### How to Use:
        1. **Select a Model**: The models mimic a typical serch engine and are trained on different approaches for understanding word similarities.
        2. **Enter Query**: Your query can be related to clinical trials, medical research, or any other relevant information.
        3. **View Results**: After entering your query, the tool will return the top 10 most relevant clinical trials from the database based on their similarity to your search.
...
""", unsafe_allow_html=True)
    ...
    if query:
        ...
        fig = go.Figure(data=[go.Table(header=dict(values=['ID', 'Title','Abstract','Publication Date','Similarity Score']),cells=dict(values=[list(P['Trial ID'].values),list(P['Title'].values), list(P['Abstract'].values),list(P['Publication date'].values),list(np.around(sim,4))],align=['center','right']))])
    ...
    # Footer
    st.markdown("---")  # A horizontal line for separation
    st.markdown("**Created by Shola Ayeotan**")
